 ...
 * actually being deleted. `folders`/`affirmations`/`goals` cascade-delete
 * remotely via their user_id FK once the auth.users row is gone (see the
 * migrations), so only local cleanup is needed after that: SQLite rows,
 * on-device audio/image files, and any now-orphaned sync_queue entries for
 * this user's rows.
 */
export async function deleteAccount(userId: string): Promise<void> {
  ...
  const [folderRows, affirmationRows, goalRows] = await Promise.all([
